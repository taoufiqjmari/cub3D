...
void	cast_ray(float ray_ang, int strip_id)
{
	...
	*/
	...
	while (next_horz_touch_x >= 0 && next_horz_touch_x <= g_elements.map_width * TILE_SIZE && next_horz_touch_y >= 0 && next_horz_touch_y <= g_elements.map_height * TILE_SIZE)
	{
		...
		if (rays_map_has_wall_at(x_to_check, y_to_check))
		{
			...
			horz_wall_content = g_elements.map[(int)floor(y_to_check / TILE_SIZE)][(int)floor(x_to_check / TILE_SIZE)];
	...
	*/
	...
	// Increment xstep and ystep until we find a wall
	while (next_vert_touch_x >= 0 && next_vert_touch_x <= g_elements.map_width * TILE_SIZE && next_vert_touch_y >= 0 && next_vert_touch_y <= g_elements.map_height * TILE_SIZE)
	{
		...
		if (rays_map_has_wall_at(x_to_check, y_to_check))
		{
			...
			vert_wall_content = g_elements.map[(int)floor(y_to_check / TILE_SIZE)][(int)floor(x_to_check / TILE_SIZE)];
